...
class Aggregate(nn.Module):
    def __init__(self, len_feature):
        ...
        self.bn5_adv = nn.BatchNorm1d(2048)

        self.non_local = NONLocalBlock1D(512, sub_sample=False, bn_layer=False)



    def forward(self, x,advbatch=False):
        if advbatch:
            ...
            out1 = self.bn1_adv(out1)
            out2 = self.conv_2(out)
            out2 = self.bn2_adv(out2)
            out3 = self.conv_3(out)
            out3 = self.bn3_adv(out3)
            out_d = torch.cat((out1, out2, out3), dim = 1)

            out4 = self.conv_4(out)
            #print(out.shape, 'out')
            out4_nl = self.non_local(out4)
            out4_nl = self.bn4_adv(out4_nl)
            out4 = out4 + out4_nl
            #print(out.shape, 'out nonlocal')
            out = torch.cat((out_d, out4), dim=1)
            out = self.conv_5(out)   # fuse all the features together
            out = self.bn5_adv(out)
            out = out + residual
...
class Model(nn.Module):
    ...
    def forward(self, inputs,advbatch=False):
        ...
        out = self.Aggregate(out,advbatch)
        #print(out.shape,'out')
       ...
       # print(afea_magnitudes.shape,'afea')


        if afea_magnitudes.shape[0] == 0:  # this is for inference

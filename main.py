...
viz = Visualizer(env='shanghai tech', use_incoming_socket=False)

if __name__ == '__main__':
    os.environ['CUDA_VISIBLE_DEVICES'] = '0'
    args = option.parser.parse_args()
    config = Config(args)
    train_nloader = DataLoader(Dataset(args, test_mode=False, is_normal=True),  #normal samples
                               batch_size=args.batch_size, shuffle=True,
                               num_workers=0, pin_memory=False, drop_last=True)

    train_aloader = DataLoader(Dataset(args, test_mode=False, is_normal=False),   #abnormal samples
                               batch_size=args.batch_size, shuffle=True,
                               num_workers=0, pin_memory=False, drop_last=True)
    ...
    for step in tqdm(
        ...
        # dir = '/home/heping/Downloads/MIL/RTFM-main/AUC'
